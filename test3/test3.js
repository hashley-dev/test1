...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function makeArt(title, subtitle, w, h, hue) {
    ...
    const [bg, accent] = palette[hue] || palette.red;
    // Ảnh nền KHÔNG còn chữ: title/subtitle vẫn nhận vào để giữ nguyên chữ ký
    // hàm (mọi lời gọi makeArt hiện có không phải sửa), nhưng không vẽ ra nữa.
    // Chữ tiêu đề đã có sẵn ở phần nội dung HTML đè lên ảnh nền.
    const svg =
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}">` +
      `<rect width="${w}" height="${h}" fill="${bg}"/>` +
      `<rect x="12" y="12" width="${w - 24}" height="${h - 24}" fill="none" stroke="${accent}" stroke-width="2" opacity="0.5"/>` +
      `<circle cx="${w / 2}" cy="${h / 2}" r="${Math.min(w, h) * 0.3}" fill="${accent}" opacity="0.07"/>` +
      `</svg>`;
  ...
  const heroesData = [
    {
      ...
      // Ảnh lấy từ URL bên ngoài. Muốn tự chứa: tải về images/quang-trung.jpg rồi
      // đổi thành image: "images/quang-trung.jpg".
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT7nD3ECghK-ZJsR6PyC3ePvE94iBb3QMNT1yOU4nkDWQ&s=10",
    ...
    },
    {
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRMUVRoeuxqpUee43vrwsYRhhNaf3ncW8ERchvCjpGPcQ&s=10",
    ...
    },
    {
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQuWQpm5RDvek4LjOCo_DzHsNqAVdIqhO_NhvHGb7zCuA&s=10",
      imageFallback: makeArt(
        "Trần Quang|Diệu",
        "Đại tướng Tây Sơn",
        600,
    ...
    },
    {
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRaV532Ghe6ZxyRlNDa90OQ-x8dUnRAjr3uSkMFJz5s1Q&s=10",
      imageFallback: makeArt(
        "Bùi Thị|Xuân",
        "Nữ tướng Tây Sơn",
        600,
    ...
    },
    {
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcScig8Esaxl3B1jfDB4gc7sMGwAV211wIjvQyRGT4IsfA&s=10",
    ...
    },
    {
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvSBnC_4nq2klGG79MFnRifFLj5HS3a4g1XvVnykIF9w&s=10",
      imageFallback: makeArt(
  ...
  // Mục đầu lấy ảnh từ URL ngoài (trùng với chân dung Quang Trung ở mục Nhân vật);
  // các mục còn lại đọc file trong images/ — thiếu file thì hiện ảnh SVG dự phòng.
  const galleryData = [
    {
      src: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT7nD3ECghK-ZJsR6PyC3ePvE94iBb3QMNT1yOU4nkDWQ&s=10",
    ...
    {
      // Tượng đài tại Bảo tàng Quang Trung (Wikimedia Commons, giấy phép tự do).
      // Đường dẫn hash của Wikimedia phải giữ đúng HAI tầng: /commons/ + <b>/ + <b7>/.
      // Ghép chuỗi để không bị gộp thành một tầng khi sửa file.
      src:
